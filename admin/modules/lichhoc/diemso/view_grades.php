...
$hocVienResult = $conn->query("SELECT hv.id_hocvien, hv.ten_hocvien, hv.email FROM hocvien hv JOIN dangkykhoahoc dk ON hv.id_hocvien = dk.id_hocvien WHERE dk.id_lop = '$lop_id' ORDER BY hv.ten_hocvien ASC");
...
<style>
    ...
    .grade-table th .delete-column-btn {
        cursor: pointer;
        color: #ffc107;
        font-size: 0.8em;
        margin-left: 8px;
    ...
    .grade-table th:hover .delete-column-btn {
        visibility: visible;
        opacity: 1;
    }

    .grade-table th .delete-column-btn:hover {
        color: #dc3545;
    }
    ...
    .grade-table .form-control:focus {
        ...
        box-shadow: 0 0 0 2px rgba(13, 179, 59, 0.2);
    }

    .grade-table .comment-icon {
        cursor: pointer;
        color: #6c757d;
    }

    .grade-table .comment-icon:hover,
    .grade-table .comment-icon.has-comment {
...
<div class="card animated-card">
    ...
    <div class="card-body">
                ...
                <div class="grade-table-wrapper">
                    <table class="table table-bordered table-hover text-center grade-table" id="grade-table">
                        <thead class="table-info">
                            <tr>
                                <th class="text-start student-col">Học viên</th>
                        ...
                        <tbody>
                            <?php foreach ($hocVien as $hv): ?>
                                <tr data-student-id="<?php echo $hv['id_hocvien']; ?>">
                                    <td class="text-start sticky-col"><strong><?php echo htmlspecialchars($hv['ten_hocvien']); ?></strong> <br>
                                    ...
                                    ?>
                                        <td>
                                            <div class="input-group input-group-sm justify-content-center">
                                                <input type="text" name="grades[<?php echo $hv['id_hocvien']; ?>][<?php echo htmlspecialchars($ld['loai_diem']); ?>][diem]" class="form-control" value="<?php echo htmlspecialchars($diem); ?>">
                                                <button class="btn btn-outline-secondary comment-icon <?php if (!empty($nhan_xet)) echo 'has-comment'; ?>" type="button" title="Thêm/Sửa nhận xét" data-student-id="<?php echo $hv['id_hocvien']; ?>" data-grade-type="<?php echo htmlspecialchars($ld['loai_diem']); ?>">
                                                    <i class="fa-solid fa-comment-dots"></i>
                                                </button>
                                                <input type="hidden" class="comment-hidden-input" id="comment-<?php echo $hv['id_hocvien']; ?>-<?php echo htmlspecialchars(preg_replace('/[^a-zA-Z0-9]/', '_', $ld['loai_diem'])); ?>" name="grades[<?php echo $hv['id_hocvien']; ?>][<?php echo htmlspecialchars($ld['loai_diem']); ?>][nhan_xet]" value="<?php echo htmlspecialchars($nhan_xet); ?>">
                                            </div>
                                        </td>
                                    <?php endforeach; ?>
                                </tr>
        ...
        <?php else: ?>
            <div class="alert alert-light text-center">Chưa có học viên nào trong lớp để chấm điểm.</div>
        <?php endif; ?>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <textarea id="commentText" class="form-control" rows="5" placeholder="Nhập nhận xét của bạn..."></textarea>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            fetch('./modules/lichhoc/diemso/delete_grades_column.php', {
                                    ...
                                    body: JSON.stringify({
                                        id_lop: '<?php echo $lop_id; ?>',
                                        loai_diem: gradeTypeToDelete
                                    })
                                })
                                .then(res => res.json())
                                .then(data => {
        ...
        document.getElementById('add-grade-type-btn').addEventListener('click', function() {
            ...
            if (newGradeType === '') {
                Swal.fire('Lỗi', 'Vui lòng nhập tên cột điểm mới.', 'error');
                return;
            }
            ...
            if (headerRow.querySelector(`th[data-grade-type="${newGradeType}"]`)) {
                Swal.fire('Lỗi', 'Tên cột điểm này đã tồn tại.', 'error');
                return;
            }
