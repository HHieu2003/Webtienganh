...
<head>
    ...
    <style>
        ...
        .course-detail-page { background: var(--course-bg); }
        .course-hero { position: relative; padding: 90px 0 70px; overflow: hidden; background: linear-gradient(145deg, #e8f8f0 0%, #c7ecd9 55%, #f8fffb 100%); }
        .hero-pattern { position: absolute; inset: 0; background-image: radial-gradient(circle at 20% 20%, rgba(13, 179, 59, 0.08), transparent 60%), radial-gradient(circle at 80% 0%, rgba(5, 101, 73, 0.12), transparent 55%); opacity: 0.9; pointer-events: none; }
        .course-hero__info { position: relative; z-index: 2; color: var(--course-secondary); }
        .course-hero__badge { display: inline-flex; align-items: center; gap: 8px; padding: 8px 18px; border-radius: 999px; background: rgba(255, 255, 255, 0.75); color: var(--course-secondary); font-weight: 600; font-size: 0.9rem; box-shadow: 0 12px 30px rgba(13, 179, 59, 0.15); margin-bottom: 18px; }
        .course-hero__title { font-size: clamp(2.4rem, 3vw, 3.4rem); font-weight: 800; line-height: 1.1; margin-bottom: 16px; color: #024429; }
        .course-hero__subtitle { font-size: 1.05rem; line-height: 1.8; color: var(--course-muted); max-width: 560px; margin-bottom: 28px; }
        .course-hero__meta { list-style: none; padding: 0; margin: 0 0 32px; display: flex; flex-wrap: wrap; gap: 16px; }
        .course-hero__meta li { display: inline-flex; align-items: center; gap: 10px; padding: 10px 18px; border-radius: 14px; background: rgba(255, 255, 255, 0.85); color: var(--course-secondary); font-weight: 600; box-shadow: 0 10px 24px rgba(4, 68, 47, 0.1); }
        .course-hero__meta i { color: var(--course-primary); }
        .course-hero__cta { display: flex; flex-wrap: wrap; gap: 16px; align-items: center; }
        .btn-enroll { display: inline-flex; align-items: center; justify-content: center; gap: 10px; padding: 14px 26px; border-radius: 14px; border: none; background: linear-gradient(135deg, var(--course-primary) 0%, var(--course-primary-dark) 100%); color: #fff; font-weight: 700; font-size: 1rem; text-transform: uppercase; letter-spacing: 0.5px; box-shadow: 0 15px 30px rgba(13, 179, 59, 0.3); transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .btn-enroll i { transition: transform 0.25s ease; }
        .btn-enroll:hover { transform: translateY(-4px); box-shadow: 0 18px 32px rgba(13, 179, 59, 0.35); color: #fff; }
        .btn-enroll:hover i { transform: translateX(5px); }
        .course-hero__outline { display: inline-flex; align-items: center; gap: 10px; padding: 12px 22px; border-radius: 14px; font-weight: 600; color: var(--course-secondary); background: rgba(255, 255, 255, 0.75); box-shadow: 0 12px 24px rgba(4, 68, 47, 0.08); text-decoration: none; transition: transform 0.2s ease, box-shadow 0.2s ease; }
        .course-hero__outline:hover { transform: translateY(-3px); box-shadow: 0 16px 30px rgba(4, 68, 47, 0.12); color: var(--course-secondary); }
        .course-hero__visual { position: relative; z-index: 2; }
        .course-hero__grid > [class*='col-'] { margin-bottom: 40px; }
        @media (min-width: 992px) { .course-hero__grid > [class*='col-'] { margin-bottom: 0; } }
        .course-hero__image { position: relative; border-radius: 28px; overflow: hidden; background: linear-gradient(160deg, rgba(4, 68, 47, 0.1), rgba(4, 68, 47, 0.25)); padding: 16px; box-shadow: 0 25px 60px rgba(8, 63, 42, 0.25); }
        .course-hero__image img { width: 100%; display: block; border-radius: 20px; object-fit: cover; aspect-ratio: 4/3; }
        .course-hero__floating-card { position: absolute; bottom: 24px; left: 24px; padding: 18px 20px; border-radius: 16px; background: rgba(255, 255, 255, 0.92); backdrop-filter: blur(6px); box-shadow: 0 20px 40px rgba(4, 68, 47, 0.2); display: grid; gap: 6px; max-width: 220px; }
        .floating-card__label { font-size: 0.85rem; font-weight: 600; color: var(--course-primary-dark); text-transform: uppercase; letter-spacing: 0.8px; }
        .floating-card__score { font-size: 1.1rem; font-weight: 700; color: #024429; display: inline-flex; align-items: center; gap: 8px; }
        .floating-card__score i { color: #ffb400; }
        .floating-card__reviews { font-size: 0.85rem; color: var(--course-muted); margin: 0; }
        .floating-card__price { font-weight: 700; font-size: 1.05rem; color: var(--course-secondary); }
        .course-highlights { padding: 50px 0; margin-top: -40px; }
        .course-highlights__row > [class*='col-'] { margin-bottom: 24px; }
        @media (min-width: 992px) { .course-highlights__row > [class*='col-'] { margin-bottom: 0; } }
        .highlight-card { background: var(--course-surface); border-radius: 22px; padding: 24px; box-shadow: 0 18px 40px rgba(0, 0, 0, 0.05); border: 1px solid var(--course-border); transition: transform 0.25s ease, box-shadow 0.25s ease; height: 100%; position: relative; overflow: hidden; }
        .highlight-card::after { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(13, 179, 59, 0.07), transparent 65%); opacity: 0; transition: opacity 0.25s ease; }
        .highlight-card:hover { transform: translateY(-8px); box-shadow: 0 24px 50px rgba(13, 179, 59, 0.15); }
        .highlight-card:hover::after { opacity: 1; }
        .highlight-card__icon { width: 48px; height: 48px; border-radius: 16px; background: rgba(13, 179, 59, 0.15); display: flex; align-items: center; justify-content: center; color: var(--course-primary); font-size: 1.2rem; margin-bottom: 14px; }
        .highlight-card__label { font-size: 0.95rem; font-weight: 600; color: var(--course-muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 6px; }
        .highlight-card__value { font-size: 1.05rem; font-weight: 700; color: #024429; margin: 0; line-height: 1.6; }
        .course-main-section { padding: 30px 0 90px; }
        .course-main-row > [class*='col-'] { margin-bottom: 40px; }
        @media (min-width: 992px) { .course-main-row > [class*='col-'] { margin-bottom: 0; } }
        .course-tabs { background: var(--course-surface); border-radius: 24px; padding: 30px; box-shadow: 0 25px 55px rgba(0, 0, 0, 0.08); border: 1px solid var(--course-border); }
        .course-tabs__nav { border-bottom: 1px solid rgba(4, 68, 47, 0.12); margin-bottom: 24px; gap: 12px; }
        .course-tabs__nav .nav-link { border: none; border-radius: 14px; padding: 12px 22px; font-weight: 600; color: var(--course-muted); background: transparent; transition: all 0.2s ease; }
        .course-tabs__nav .nav-link.active, .course-tabs__nav .nav-link:hover { background: rgba(13, 179, 59, 0.12); color: var(--course-secondary); }
        .course-tabs__content .tab-pane { animation: fadeSlideUp 0.4s ease; }
        .course-description { font-size: 1rem; line-height: 1.9; color: #3d4b44; }
        .reviews-section { display: grid; gap: 32px; }
        .reviews-header { display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 20px; }
        .reviews-score { background: rgba(13, 179, 59, 0.08); border-radius: 18px; padding: 18px 24px; display: inline-flex; align-items: center; gap: 18px; color: var(--course-secondary); }
        .reviews-score .score-value { font-size: 2.4rem; font-weight: 800; line-height: 1; color: #024429; }
        .score-stars { display: flex; gap: 6px; font-size: 1.05rem; }
        .score-stars .is-active { color: #ffb400; }
        .reviews-list { display: grid; gap: 20px; }
        .review-card { display: flex; gap: 18px; padding: 22px; border-radius: 20px; background: #f9fdfb; border: 1px solid rgba(13, 179, 59, 0.12); box-shadow: 0 15px 35px rgba(0, 0, 0, 0.06); }
        .review-card__avatar img { width: 64px; height: 64px; border-radius: 50%; object-fit: cover; border: 2px solid rgba(13, 179, 59, 0.3); }
        .review-card__header { display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 12px; }
        .review-card__header h6 { margin: 0; font-weight: 700; color: #024429; }
        .review-card__stars { display: flex; gap: 4px; color: #d9e2dd; font-size: 0.95rem; }
        .review-card__stars .is-active { color: #ffb400; }
        .review-card__content { margin: 0; color: #3d4b44; line-height: 1.7; }
        .review-card__actions { margin-top: 14px; }
        .review-card__delete { display: inline-flex; align-items: center; gap: 8px; background: none; border: none; color: #c0392b; font-weight: 600; cursor: pointer; padding: 0; transition: color 0.2s ease; }
        .review-card__delete:hover { color: #922b21; }
        .empty-state { text-align: center; padding: 40px 20px; border-radius: 20px; background: rgba(13, 179, 59, 0.08); color: var(--course-secondary); }
        .empty-state i { font-size: 2rem; margin-bottom: 14px; display: block; }
        .review-form { position: relative; border-radius: 24px; background: #f5fbf7; padding: 28px; border: 1px solid rgba(13, 179, 59, 0.15); box-shadow: 0 20px 45px rgba(0, 0, 0, 0.05); }
        .review-form__header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
        .review-form__header h5 { margin: 0; font-weight: 700; color: #024429; }
        .review-form__status { display: inline-flex; align-items: center; gap: 8px; font-size: 0.95rem; font-weight: 600; color: var(--course-secondary); background: rgba(13, 179, 59, 0.15); padding: 6px 14px; border-radius: 999px; }
        .review-form__hint { font-size: 0.95rem; color: var(--course-muted); margin-bottom: 18px; }
        .review-form.is-locked::after { content: ''; position: absolute; inset: 0; background: rgba(245, 251, 247, 0.75); border-radius: inherit; pointer-events: none; }
        .review-form__locked-message { margin-top: 18px; font-size: 0.95rem; color: var(--course-secondary); }
        .review-form__locked-message a { color: var(--course-primary-dark); font-weight: 600; text-decoration: underline; }
        .review-form textarea { resize: vertical; min-height: 140px; }
        .form-select, .review-form .form-control { border-radius: 12px; border: 1px solid rgba(13, 179, 59, 0.2); padding: 0.75rem 0.9rem; box-shadow: none; color: #3d4b44; }
        .form-select:focus, .review-form .form-control:focus { border-color: var(--course-primary); box-shadow: 0 0 0 0.2rem rgba(13, 179, 59, 0.12); }
        .form-label { font-weight: 600; color: #024429; margin-bottom: 6px; }
        .sidebar-sticky { position: sticky; top: 110px; }
        .course-summary-card { background: var(--course-surface); border-radius: 24px; padding: 30px; box-shadow: 0 25px 55px rgba(0, 0, 0, 0.08); border: 1px solid var(--course-border); display: grid; gap: 24px; }
        .course-summary-card .btn-enroll { width: 100%; }
        .course-summary-card__header { display: flex; justify-content: space-between; align-items: center; gap: 12px; }
        .course-summary-card__title { font-size: 1.05rem; font-weight: 700; color: #024429; text-transform: uppercase; letter-spacing: 1px; }
        .course-summary-card__price { font-weight: 800; font-size: 1.1rem; color: var(--course-secondary); }
        .summary-list { list-style: none; padding: 0; margin: 0; display: grid; gap: 16px; }
        .summary-list li { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 12px; padding-bottom: 12px; border-bottom: 1px dashed rgba(13, 179, 59, 0.15); color: #3d4b44; font-weight: 600; }
        .summary-list li:last-child { border-bottom: none; padding-bottom: 0; }
        .summary-list i { color: var(--course-primary); font-size: 1.1rem; }
        .summary-list span { color: var(--course-muted); font-weight: 500; }
        .summary-list strong { color: #024429; }
        .course-summary-card__note { display: flex; gap: 12px; align-items: flex-start; background: rgba(13, 179, 59, 0.08); border-radius: 16px; padding: 16px; color: var(--course-secondary); }
        .course-summary-card__note i { font-size: 1.2rem; }
        .modal-content { border-radius: 22px; border: none; overflow: hidden; }
        .modal-header { background: #f1fbf6; border-bottom: 1px solid rgba(13, 179, 59, 0.12); }
        .modal-title { font-weight: 700; color: #024429; }
        .modal-body { background: #f8fbf9; padding: 28px; }
        .class-item { background: var(--course-surface); border-radius: 18px; padding: 22px; border: 1px solid rgba(13, 179, 59, 0.12); box-shadow: 0 18px 40px rgba(0, 0, 0, 0.05); margin-bottom: 18px; animation: slideInUp 0.5s ease forwards; opacity: 0; }
        .class-header { display: flex; align-items: center; justify-content: space-between; gap: 14px; flex-wrap: wrap; }
        .class-header h6 { margin: 0; font-weight: 700; color: var(--course-secondary); }
        .class-actions .btn { border-radius: 10px; font-weight: 600; }
        .class-meta { display: flex; gap: 18px; color: var(--course-muted); font-size: 0.95rem; margin-top: 10px; flex-wrap: wrap; }
        .schedule-list { list-style: none; margin: 0; padding: 0; max-height: 220px; overflow-y: auto; }
        .schedule-list li { display: flex; justify-content: space-between; gap: 12px; padding: 12px 16px; border-bottom: 1px solid rgba(13, 179, 59, 0.08); font-size: 0.95rem; flex-wrap: wrap; }
        .schedule-list li:last-child { border-bottom: none; }
        @keyframes slideInUp { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
        @keyframes fadeSlideUp { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        @media (max-width: 991.98px) { .course-hero { padding: 70px 0 60px; } .course-hero__meta li { width: 100%; } .course-highlights { margin-top: -20px; } .course-tabs { padding: 24px; } }
        @media (max-width: 767.98px) { .course-hero__meta { flex-direction: column; } .course-hero__floating-card { position: static; margin-top: 18px; } .course-hero__image { padding: 12px; } .course-tabs { padding: 20px; } .course-summary-card { padding: 24px; } }
    </style>
</head>
<body>
